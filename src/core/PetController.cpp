    ...
    , m_studyWindow(new StudyWindow)
{
    ...
    connect(m_petWindow, &PetWindow::settingsRequested, this, [this] { m_userData->settings.autoStart=AutoStartManager::isEnabled();m_settingsWindow->setSettings(m_userData->settings); m_settingsWindow->show(); m_settingsWindow->raise(); });

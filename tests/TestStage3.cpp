...
class TestStage3 final: public QObject { Q_OBJECT private slots: void settingsClamp(); void statsRewardsAreBounded(); void clickThrottle(); void pomodoroCanBeDrivenWithoutWaiting(); void storageRoundTripAndTodo(); void corruptJsonFallsBack(); void todoEditAndDeadlineRoundTrip(); void emptyTodoIsRejected(); void autoStartQuoting(); };
...
void TestStage3::todoEditAndDeadlineRoundTrip(){QTemporaryDir dir;StorageService store(dir.filePath("data.json"));UserData d;TodoItem t{QString(),"初稿",false,QDateTime(),QDate(2026,10,1)};StorageService::upsertTodo(d,t);auto id=d.todos.first().id;t.id=id;t.title="终稿";t.completed=true;StorageService::upsertTodo(d,t);QVERIFY(store.save(d));auto r=store.load();QCOMPARE(r.todos.size(),1);QCOMPARE(r.todos.first().title,QString("终稿"));QCOMPARE(r.todos.first().dueDate,QDate(2026,10,1));QVERIFY(r.todos.first().completed);}
void TestStage3::emptyTodoIsRejected(){UserData d;StorageService::upsertTodo(d,{QString(),QString()});QCOMPARE(d.todos.size(),0);}
void TestStage3::autoStartQuoting(){QCOMPARE(AutoStartManager::quotedExecutable("C:/Program Files/Study Pet/StudyPet.exe"),QString("\"C:\\Program Files\\Study Pet\\StudyPet.exe\""));}
